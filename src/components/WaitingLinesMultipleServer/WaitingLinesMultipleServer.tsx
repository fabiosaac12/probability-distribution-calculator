...
import { useStyles } from "./WaitingLinesMultipleServerStyles";
import { round } from "../../helpers/maths/round";

export const WaitingLinesMultipleServer: FC = () => {
          ...
          <div className={classes.form}>
            ...
            <Box className={classes.input} mb={2}>
              <TextField
                ...
                onBlur={form.handleBlur}
                value={form.values.c}
                error={form.touched.c && !!form.errors.c}
                helperText={form.touched.c && form.errors.c}
              />
            </Box>
                ...
                <div>
                  ...
                  {result.activeServers ? (
                    <Typography variant="subtitle1">
                      Servidores activos:{" "}
                      <span className={classes.resultValue}>
                        {round(result.activeServers)}
                      </span>
                    </Typography>
                  ) : null}
                  {result.inactiveServers ? (
                    <Typography variant="subtitle1">
                      Servidores inactivos:{" "}
                      <span className={classes.resultValue}>
                        {round(result.inactiveServers)}
                      </span>
                    </Typography>
                  ) : null}
                </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {result.probabilities.map((row, index) => (
                        <TableRow key={`waiting-lines-multiple-server-row-probability-${index}`}>
